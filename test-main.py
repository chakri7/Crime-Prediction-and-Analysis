...
def women():
    ...
    # Used 65% accuracy as benchmark for trending year finding algorithm.
    if(accuracy < 0.65):
        for a in range(3, l-8):
            ...
            if (accuracy > accuracy_max):
                ...
                print(accuracy_max)
                trendChangingYear = a
    # Printing Trend Changing Year on server terminal.
...
def children():
    ...
    # reading CSV file.
    df = pd.read_csv(
        "static/Statewise Cases Reported of Crimes Committed Against Children 1994-2016.csv", header=None)
    ...
    xTrain = np.array([1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,
                       2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016])
    yTrain = test[2:25]

    X = df.iloc[0, 2:l].values
    y = test[2:]
    regressor = RandomForestRegressor(n_estimators = 100, random_state = 0)  # regression Algorithm Called.
    # Data set is fitted in regression and Reshaped it.
    regressor.fit(X.reshape(-1, 1), y)
    # Finding Accuracy of Prdictions.
    accuracy = regressor.score(X.reshape(-1, 1), y)
    print(accuracy)
    accuracy_max = 0.65
    if(accuracy < 0.65):
...
def ipc():
    ...
    xTrain = np.array([2001, 2002, 2003, 2004, 2005, 2006, 2007,
